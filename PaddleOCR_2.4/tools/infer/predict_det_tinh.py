...
import tools.infer.utility_tinh as utility
from ppocr.utils.logging import get_logger
from ppocr.utils.utility import get_image_file_list, check_and_read_gif
from ppocr.data import create_operators, transform
from ppocr.postprocess import build_post_process_tinh
...
class TextDetector(object):
    def __init__(self, args):
        ...
        self.postprocess_op = build_post_process_tinh(postprocess_params)
...
def process_dict_list(dic_list):
...
if __name__ == "__main__":
    ...
    #args.image_dir = '/content/public_test_img'
    #args.det_model_dir = '/content/drive/MyDrive/BusMap_Private1/OCR_BKAI/Paddle_det_res18_db_v2.0_ver_4'
    #args.det_db_thresh = 0.3
    #args.det_db_box_thresh = 0.3
    #args.use_gpu = False
    #args.det_out_txt_dir = '/content/test_output'
    det_visualize = True
    #det_out_txt_dir = '/content/test_output'
    #det_out_viz_dir = '/content/test_output'
    ...
    for image_file in image_file_list:
        ...
        annotation_string = process_dict_list(result_list)
        save_pred = image_file + "\t" + annotation_string + "\n"
        ...
        output_txt_path = os.path.join(draw_img_save, img_name_pure.replace('.jpg', '.txt'))
        src_im = utility.draw_text_det_res(dt_boxes, image_file, save_path=output_txt_path)
        if det_visualize:
            img_path = os.path.join(draw_img_save, "{}".format(img_name_pure))
            cv2.imwrite(img_path, src_im)
            logger.info("The visualized image saved in {}".format(img_path))
    if count > 1:
        logger.info("Avg Time: {}".format(total_time / (count - 1)))
    with open(os.path.join(draw_img_save,"det.txt"), 'w') as f:
        f.writelines(save_results)
        f.close()
    with open('/content/missed_detection/miss_img.txt', 'w') as f:
          f.writelines(missed_images)
          f.close()
